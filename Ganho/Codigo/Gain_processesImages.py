...
"""
    ...
    Descricao: esta biblioteca possui as seguintes funcoes:
        parametrosCaixaPixels: esta funcao recebe o nome de uma unica imagens, retornando os valores da coordenada central
        e dimensao da caixa de pixels.
        ...
        recebeListaImgs_geraArqListaCombinacoes: esta funcao recebe uma lista de imagens flat e o numero de imagens adquiridas
        para a mesma intensidade de luz. Para cada conjunto de n imagens, adiciona a um vetor as possiveis combinacoes entre pares
        detes conjunto atraves da funcao organizaCombinacoes; ao terminar os conjuntos, a funcao escreve em um arquivo chamado
        arquivoListaCombinacoes os pares de imagens separados em colunas.

        LeArqImgsFlat_retornaListaImgs: esta funcao faz a leitura do arquivo arquivoListaCombinacoes criado pela funcao
        recebeListaImgs_geraArqListaCombinacoes, retornando uma lista com o nome dessas imagens.

        calcXY_YerrorBar_XerrorBar: esta funcao recebe uma lista de imagens flat, uma lista de imagens bias e o numero de imagens
        adquiridas para cada patamar de intensidade de luz. Ela realiza o calculo da intensidade do sinal (em eletrons) e do valor
        da variancia comparando duas imagens de bias e duas imagens de flat. Para isso, faz a chamada da funcao
        recebeListaImgs_geraArqListaCombinacoes que tomara as imagens obtidas e, dado o numero de imagens em cada conjunto, retorna
        uma lista de duas colunas com cada par de combinações possíveis. Para cada par de imagens é retirada uma caixa de pixels de
        100 x 100 no centro da imagem; sobre essa caixa será calculada a intensidade do sinal (EixoY), a variancia (EixoX), o desvio
        padrao das medidas (StdEixoY) e a barra de erro para os valores em X.
...
#----------------------------------------------------------------------------------------------------------------------------------
def calcXY_YerrorBar_XerrorBar(listaFlat, listaBias, numeroImagens, parametersBox, images_path): 
    coordx = parametersBox[0]
    coordy = parametersBox[1]
    b = int(parametersBox[2])    
    X, Y, SigmaTotal = [], [], []
    ...
    divisor = len(listaBias)

    #le imagens de flat
    for i in range(len(listaImgs))[::2]:        
        FlatA = fits.getdata(images_path + '\\' + listaImgs[i])[0]
        FlatB = fits.getdata(images_path + '\\' + listaImgs[i+1])[0]
        BiasA = fits.getdata(images_path + '\\' + listaBias[i%divisor])[0]
        BiasB = fits.getdata(images_path + '\\' + listaBias[(i+1)%divisor])[0]  
