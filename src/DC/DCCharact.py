...
class DarkCurrent:
	...
	def iniciaObjeto(self):
		...
		self.cwd = os.getcwd()
		self.Keywords = [options.bias,options.dark]	
		self.KeyWordDirectories = options.directories
		self.ganho = float(options.gain)
		self.Dic = {'qtdImagesBias':0, 'qtdImagesDark':0, 'minute':self.minute, 'second':self.second, 'tagBias':options.bias, 'tagDark':options.dark}

		def criaArq_listaDirectories():
			s=''	
			listaArquivos = os.listdir(self.cwd)
			listaArquivos.sort()
	...
	def caractTemporal(self):
		for Dir in self.Directories:
			...
			listaImgBias = DCReadArq.returnListImages(options.bias)	
	...
	def DiretorioMenorTemperatura(self):
		...
		os.chdir(self.cwd + '/' + DiretorioTempMin)					
	

	def CaractTemporal(self):
		self.DCnominal = DCvariacaoTemporal.caractTemporal(self.cwd, self.Directories, self.vetorTemp, self.ganho)	
...
DCobjeto = 	DarkCurrent()
DCobjeto.iniciaObjeto()
