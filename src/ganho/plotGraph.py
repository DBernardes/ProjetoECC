...
def Graph_sinal_variance(X, Y, std, Estd):
	...
	plt.ylabel(r'$\mathtt{Intensidade \quad do \quad sianl \quad (adu/pixel)}$', size=font)
...
def Graph_residuos(x,y, std):
	...
	plt.xlabel(r'$\mathtt{Vari\^ancia \quad (adu)}$', size=font)
	plt.ylabel(r'$\mathtt{Intensidade \quad do \quad sinal \quad (adu/pixel)}$', size=font)
