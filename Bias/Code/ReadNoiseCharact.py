...
"""
    ...
    Descricao: este modulo visa reunir as bibliotecas responsaveis para a caracterizacao do ruido de leitura:
    variacaoTemporal, geraArquivo, gradiente, biashistogram, CCDinfo, caixaTexto, logfile, makeList_imagesInput.
    O codigo ira criar um arquivo contendo a lista de nomes das imagens; dessa lista ira retirar uma amostra de 10
    imagens e retorna-la para as funcoes gradiente e histograma. A funcao gradiente ira expressar em um grafico em
    cores a variacao das contagens dos pixels ao longo desta imagem mediana, junto de dois outros graficos da variacao
    da media das constagens ao longo de suas linhas e colunas; a funcao histograma ira calcular uma distribuicao de
    frequencia das contagens para um intervalo da media dos pixels +/- 7sigmas. A funcao variacaoTemporal ira receber
    a lista de imagens completa, retornando um grafico com a mediana das contagens de cada imagem em funcao do tempo
    do experimento. Sobre esses dados e realizada uma FFT; e gerada uma segunda lista de dados normais em relacao a
    media e desvio padrao dos dados originais, de modo que a FFT desta serie e plotada junto a primeira, permitindo
...
class ReadNoiseCharact:
	...
	def __init__(self, dir_path):
		self.cwd = dir_path	
		self.criaArq_listaImgInput()
		self.readArq_returnlistImages()		
		self.header = fits.getheader(self.cwd + '\\' + self.listaImagensBias[0]) #aquisicao do header
